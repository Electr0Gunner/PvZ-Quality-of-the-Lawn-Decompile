...
inline _DWORD PPM_CONTEXT::cutOff(int Order)
{
...
extern "C" int unppmdj1_raw(unsigned char *in, int insz, unsigned char *out, int outsz, int SaSize, int MaxOrder, BOOL CutOff) {
    EncodedFile  = in;
...
extern "C" int unppmdj1(unsigned char *in, int insz, unsigned char *out, int outsz) {
    if(insz < 2) return(-1);
    int         parameters = in[0] | (in[1] << 8);
    int         MaxOrder = (parameters & 0x0f) + 1;
    int         SaSize   = ((parameters >> 4) & 0xFF) + 1;
    BOOL        CutOff   = 0;
    return(unppmdj1_raw(in + 2, insz - 2, out, outsz, SaSize, MaxOrder, CutOff));
}
